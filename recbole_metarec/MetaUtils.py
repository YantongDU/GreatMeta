...
class EmbeddingTable(nn.Module):
    ...
    def embeddingSingleField(self, field, batchX, paramWeightDict=None):
        ...
        '''
        if self.fieldType(field) is FeatureType.TOKEN:
            return self.embeddingDict[field](batchX) if paramWeightDict is None else F.embedding(batchX,
                                                                                                 paramWeightDict[
                                                                                                     field])
        if self.fieldType(field) is FeatureType.TOKEN_SEQ:
            return torch.sum(self.embeddingDict[field](batchX) if paramWeightDict is None else F.embedding(batchX,
                                                                                                           paramWeightDict[
                                                                                                               field]),
                             dim=1)
        if self.fieldType(field) is FeatureType.FLOAT:
...
def metaQuickStart(modelName,datasetName, paramConfig):
    ...
    paramConfig['data_path'] = 'recbole_metarec/dataset/'
